...
RSpec.describe Sanitarios::CirugiaMenor do
  ...
  let(:equipos_requeridos_array) { Sanitarios::CirugiaMenor::EQUIPOS_REQUERIDOS }

  let(:cirugia_menor_base) do
    Sanitarios::CirugiaMenor.new(id_valido, nombre_valido, quirofanos_validos, personal_valido, precio_valido)
  end
  
  let(:equipos_requeridos_str) { 
    cirugia_menor_base.send(:_join_elements, equipos_requeridos_array) 
  }

  let(:cirugia_menor_alternativa) do 
    Sanitarios::CirugiaMenor.new(id_valido2, nombre_valido2, quirofanos_validos2, personal_validos2, precio_valido2)
  end

  context "Pruebas de Herencia y Clase" do
    it "cirugia_menor_base debe ser una instancia de CirugiaMenor" do
      expect(cirugia_menor_base).to be_an_instance_of(Sanitarios::CirugiaMenor)
      expect(cirugia_menor_base.class).to eq(Sanitarios::CirugiaMenor)
    end
    
    it "cirugia_menor_base debe heredar de ServicioQuirurgico" do
      expect(cirugia_menor_base).to be_a(Sanitarios::ServicioQuirurgico)
      expect(cirugia_menor_base).to be_kind_of(Sanitarios::ServicioQuirurgico)
    end

    it "cirugia_menor_base hereda correctamente de Object y BasicObject" do
      expect(cirugia_menor_base).to be_a(Object)
      expect(cirugia_menor_base).to be_a(BasicObject)
    end
  end
  ...
  context "Pruebas de Inicialización y Getters (Clase Hija)" do
    it "debe inicializar correctamente y devolver los atributos heredados" do
      expect(cirugia_menor_base.id).to eq(id_valido)
      expect(cirugia_menor_base.nombre).to eq(nombre_valido)
      expect(cirugia_menor_base.precio).to eq(precio_valido)
      expect(cirugia_menor_base.quirofanos.length).to eq(quirofanos_validos.length)
      expect(cirugia_menor_base.personal.length).to eq(personal_valido.length)
    end
    
    it "debe devolver el atributo nuevo 'equipo_simple' como un Array fijo de requerimientos" do
      expect(cirugia_menor_base.equipo_simple).to eq(equipos_requeridos_array)
      expect(cirugia_menor_base.equipo_simple).to be_an_instance_of(Array)
      expect(cirugia_menor_base.equipo_simple.length).to eq(2)
    end
    
    it "debe asegurar que el array de equipos asignado es una COPIA independiente y no la constante original" do
      temp_cm = Sanitarios::CirugiaMenor.new(id_valido, nombre_valido, quirofanos_validos, personal_valido, precio_valido)
      temp_cm.equipo_simple << "nuevo_equipo_extra" 
      expect(Sanitarios::CirugiaMenor::EQUIPOS_REQUERIDOS.length).to eq(2) 
      expect(temp_cm.equipo_simple.length).to eq(3) 
    end
  end
  ...
  context "Pruebas de Validación (Heredadas de ServicioQuirurgico)" do
    it "debe lanzar ArgumentError si el nombre no es un String" do
      expect { 
        Sanitarios::CirugiaMenor.new(id_valido, 12345, quirofanos_validos, personal_valido, precio_valido) 
    ...
    it "debe lanzar ArgumentError si la lista de personal está vacía" do
      expect { 
        Sanitarios::CirugiaMenor.new(id_valido, nombre_valido, quirofanos_validos, [], precio_valido) 
      }.to raise_error(ArgumentError, /personal no puede estar vacía/)
    end

    it "debe lanzar ArgumentError si el precio no es un Integer" do
      expect { 
        Sanitarios::CirugiaMenor.new(id_valido, nombre_valido, quirofanos_validos, personal_valido, nil) 
    ...
    it "debe lanzar ArgumentError si la lista de quirófanos está vacía" do
      expect { 
        Sanitarios::CirugiaMenor.new(id_valido, nombre_valido, [], personal_valido, precio_valido) 
      }.to raise_error(ArgumentError, /quirófanos no puede estar vacía/)
    end
    
    it "debe lanzar ArgumentError si el ID no es un Integer" do
      expect { 
        Sanitarios::CirugiaMenor.new("1", nombre_valido, quirofanos_validos, personal_valido, precio_valido) 
      }.to raise_error(ArgumentError, /ID debe ser un número/)
    end
  end

  context "to_s (Representación de cadena) para CirugiaMenor" do
    ...
    it "debe tener el to_s definido como publico" do
      expect(Sanitarios::CirugiaMenor.public_method_defined?(:to_s)).to be true
    end

    it "debe sobreescribir el método to_s e incluir todos los Equipos Simples Requeridos" do
      ...
      expected_output = parent_output + "\n \tEquipos Simples Requeridos: #{equipos_requeridos_str}"
      expect(cirugia_menor_base.to_s).to eq(expected_output)
    end
    
    it "debe incluir la lista completa y formateada de equipos requeridos" do
      expect(cirugia_menor_base.to_s).to include("Equipos Simples Requeridos: #{equipos_requeridos_str}")
    end
  end
  ...
  context "Pruebas de Comparación (Comparable y <=>)" do
    ...
    let(:servicio_igual) do
      Sanitarios::CirugiaMenor.new(id_igual, nombre_igual, quirofanos_igual, personal_igual, precio_igual)
    end

    let(:servicio_barato) { cirugia_menor_base } 
    let(:servicio_caro) { cirugia_menor_alternativa } 

    it "debe tener el <=> definido como publico" do
      expect(Sanitarios::CirugiaMenor.public_method_defined?(:<=>)).to be true
    end

    it "debe implementar el método <=> basado en el precio (funcionalidad heredada)" do
  ...
  context "pruebas de coste total de cirugia_mayor" do
    it "el método coste total debe estar definido como público" do
      expect(Sanitarios::CirugiaMenor.public_method_defined?(:coste_total)).to be true
    end

    it "deberia de dar el valor precio correcto" do
      expect(cirugia_menor_base.coste_total).to eq(5090)
      expect(cirugia_menor_alternativa.coste_total).to eq(15090)
    end
  end
end