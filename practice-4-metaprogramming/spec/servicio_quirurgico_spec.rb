...
RSpec.describe Sanitarios::ServicioQuirurgico do
  ...
  context "Pruebas de Funcionalidad: Costo Relativo (#disponibilidad_personal_quirofano)" do
    it "el método disponibilidad_personal_quirofano debe estar definido como público" do
      expect(Sanitarios::ServicioQuirurgico.public_method_defined?(:disponibilidad_personal_quirofano)).to be true
    end

    it "debe calcular la disponibilidad de personal por quirófano correctamente para servicio1 (2 quirófanos, 2 personas)" do
      expect(servicio1.disponibilidad_personal_quirofano).to eq(1.0)
    end

    it "debe calcular la disponibilidad de personal por quirófano correctamente para servicio2 (1 quirófano, 1 persona)" do
      expect(servicio2.disponibilidad_personal_quirofano).to eq(1.0)
    end

    it "debe manejar correctamente casos con más personal que quirófanos" do
      servicio_exceso = Sanitarios::ServicioQuirurgico.new(4, "Exceso", [q_general_1], equipo_completo_q2, 10000)
      expect(servicio_exceso.disponibilidad_personal_quirofano).to eq(4.0)
    end
  end

  context "Pruebas de Indicador: Costo Relativo por Quirófano (#indicador_costo_por_quirofano)" do
    it "el método indicador_costo_por_quirofano debe estar definido como público" do
      expect(Sanitarios::ServicioQuirurgico.public_method_defined?(:indicador_costo_por_quirofano)).to be true
    end

    it "debe calcular la ratio Costo/Quirófano correctamente para servicio1 (2 quirófanos)" do
      expect(servicio1.indicador_costo_por_quirofano).to eq(2500.0)
    end

    it "debe calcular la ratio Costo/Quirófano correctamente para servicio2 (1 quirófano)" do
      expect(servicio2.indicador_costo_por_quirofano).to eq(15000.0)
    end

    it "debe devolver un Float" do
      expect(servicio1.indicador_costo_por_quirofano).to be_an_instance_of(Float)
    end

    it "debe manejar correctamente precios altos y número de quirófanos impar" do
      servicio_impar = Sanitarios::ServicioQuirurgico.new(4, "Impar", [q_general_1, q_general_2, q_trauma_1], personal_valido, 10000)
      expect(servicio_impar.indicador_costo_por_quirofano).to be_within(0.0001).of(10000.0 / 3.0)
    end
  end

  context "Pruebas de Indicador: Costo Relativo por Personal (#indicador_costo_por_personal)" do
    it "el método indicador_costo_por_personal debe estar definido como público" do
      expect(Sanitarios::ServicioQuirurgico.public_method_defined?(:indicador_costo_por_personal)).to be true
    end

    it "debe calcular la ratio Costo/personal correctamente para servicio1 (2 personas)" do
      expect(servicio1.indicador_costo_por_personal).to eq(2500.0)
    end

    it "debe calcular la ratio Costo/personal correctamente para servicio2 (1 persona)" do
      expect(servicio2.indicador_costo_por_personal).to eq(15000.0)
    end

    it "debe manejar correctamente precios altos y número de personal impar" do
      servicio_impar = Sanitarios::ServicioQuirurgico.new(4, "Impar", [q_general_1, q_general_2, q_trauma_1], equipo_completo_q, 10000)
      servicio_impar2 = Sanitarios::ServicioQuirurgico.new(4, "Impar", [q_general_1, q_general_2, q_trauma_1], equipo_completo_q2, 10000)
      expect(servicio_impar.indicador_costo_por_personal).to be_within(0.0001).of(3333.3333333333335)
      expect(servicio_impar2.indicador_costo_por_personal).to be_within(0.0001).of(2500.0)
    end
  end
end